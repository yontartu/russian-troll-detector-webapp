...
def generate_combined_features_new_tweet(df, vocabulary):
    ...
    """
    word_vocab = vocabulary[:300]
    emoji_vocab = vocabulary[300:500]
    hashtag_vocab = vocabulary[500:800]
...
def make_prediction_on_new_tweet(text=str):
    ...
    with open('./training_vocabulary.pkl', 'rb') as f:
        vocab = pickle.load(f)
    best_rf = joblib.load('./best_rf_all_data_20190906.pkl')
    ...
    prob_to_return = str(round(100 * pred_proba, 1))

    return text, label_to_return, prob_to_return